...
set +e
timeout "${DURATION_SECONDS}s" "$BINARY" --no-audio > "$LOG_FILE" 2>&1
STATUS=$?
set -e

if [[ "$STATUS" -eq 0 ]]; then
  echo "runtime smoke failed: game exited before ${DURATION_SECONDS}s" >&2
  cat "$LOG_FILE" >&2
  exit 1
fi
...
if ! grep -q "Creating new window Bevy Open Siege" "$LOG_FILE"; then
  echo "runtime smoke failed: window creation log not found" >&2
  cat "$LOG_FILE" >&2
  exit 1
fi
